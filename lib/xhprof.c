...
typedef struct hp_function_map {
	char **names;
	uint8 filter[XHPROF_FILTERED_FUNCTION_SIZE];
} hp_function_map;
...
typedef struct hp_global_t {
	...
	uint8   trace_function_filter[XHPROF_FILTERED_FUNCTION_SIZE];
...
ZEND_BEGIN_ARG_INFO(arginfo_xhprof_transaction_name, 0)
ZEND_END_ARG_INFO()
...
zend_function_entry xhprof_functions[] = {
	...
	PHP_FE(xhprof_transaction_name, arginfo_xhprof_transaction_name)
 ...
 */
PHP_INI_ENTRY("xhprof.connection", "unix:///tmp/qprofd.sock", PHP_INI_ALL, NULL)
PHP_INI_ENTRY("xhprof.udp_connection", "127.0.0.1:8135", PHP_INI_ALL, NULL)
PHP_INI_ENTRY("xhprof.auto_start", "1", PHP_INI_ALL, NULL)
PHP_INI_ENTRY("xhprof.api_key", "", PHP_INI_ALL, NULL)
PHP_INI_ENTRY("xhprof.transaction_function", "", PHP_INI_ALL, NULL)
PHP_INI_ENTRY("xhprof.sample_rate", "10", PHP_INI_ALL, NULL)
PHP_INI_ENTRY("xhprof.load_library", "1", PHP_INI_ALL, NULL)
...
{
	long  xhprof_flags = 0;       /* Qafoo PRofiler flags */
...
PHP_FUNCTION(xhprof_layers_enable)
{
	...
	if (zend_hash_num_elements(Z_ARRVAL_P(layers)) == 0) {
		xhprof_flags = XHPROF_FLAGS_NO_USERLAND | XHPROF_FLAGS_NO_BUILTINS;
	}
...
PHP_FUNCTION(xhprof_transaction_name)
{
...
{
	if (INI_INT("xhprof.load_library") == 0) {
		return SUCCESS;
	}
	...
	if (ret == SUCCESS) {
		...
		zend_try {
			...
			php_log_err("xhprof.so: Error during execution of auto start script QafooProfiler.php - Skipping" TSRMLS_CC);
		} zend_end_try();
	}
...
PHP_MINFO_FUNCTION(xhprof)
{
	...
	php_info_print_table_row(2, "Connection (xhprof.connection)", INI_STR("xhprof.connection"));
	php_info_print_table_row(2, "UDP Connection (xhprof.udp_connection)", INI_STR("xhprof.udp_connection"));
	php_info_print_table_row(2, "Default API Key (xhprof.api_key)", INI_STR("xhprof.api_key"));
	php_info_print_table_row(2, "Default Sample-Rate (xhprof.sample_rate)", INI_STR("xhprof.sample_rate"));
	php_info_print_table_row(2, "Default Transaction Function (xhprof.transaction_function)", INI_STR("xhprof.transaction_function"));
	php_info_print_table_row(2, "Automatically Start (xhprof.auto_start)", INI_INT("xhprof.auto_start") ? "Yes": "No");
	php_info_print_table_row(2, "Load PHP Library (xhprof.load_library)", INI_INT("xhprof.load_library") ? "Yes": "No");

	const char *version = zend_get_module_version("xdebug");
	if (found != 0 && version != NULL && php_version_compare(version, "2.2.7") < 0 && INI_INT("xhprof.load_library") != 0) {
...
static inline hp_function_map *hp_function_map_create(char **names)
{
	...
	memset(map->filter, 0, XHPROF_FILTERED_FUNCTION_SIZE);
...
static char *hp_get_function_argument_summary(char *ret, zend_execute_data *data TSRMLS_DC)
{
	...
	char *oldret = ret;
	char *summary;
	int len = XHPROF_MAX_ARGUMENT_LEN;
